export default {
  header: {
    pricing: "Cennik",
    docs: "Dokumentacja",
    signIn: "Zaloguj się",
    goToApp: "Przejdź do aplikacji",
  },
  hero: {
    title: "Bezosobowe filmy dla twoich kanałów w kilka minut",
    description:
      "Przekształć swoje pomysły w przyciągające uwagę treści bez potrzeby subskrypcji. Płać tylko za filmy, które tworzysz.",
  },
  getStarted: {
    heading: "Rozpocznij",
    title: "Automatyzacja lokalizacji dla twoich aplikacji",
    description:
      "Usprawnij proces lokalizacji dzięki zasilanemu przez AI CLI i potokowi zaprojektowanemu do automatyzacji tłumaczeń dla programistów.",
    button: {
      startAutomating: "Zacznij automatyzować",
      readDocumentation: "Przeczytaj dokumentację",
    },
  },
  companies: {
    title: "Używane przez",
    addYourCompany: "+ Dodaj swoją firmę",
  },
  activity: {
    title: "Aktywność",
    loading: "Ładowanie",
    loadMore: "Pokaż więcej",
    noResults: "Brak wyników dla '{search}'",
    clearSearch: "Wyczyść wyszukiwanie",
  },
  features: {
    title: "Funkcje",
    fullyOpenSource: "W pełni open source",
    fullyOpenSourceDescription:
      "Nasz kod jest publicznie dostępny na GitHubie, umożliwiając przejrzystość i wkład społeczności.",
    noVendorLockIn: "Brak uzależnienia od dostawcy",
    noVendorLockInDescription:
      "Nie zamykamy cię w określonej usłudze, używaj własnych LLM.",
    presetsForExpo: "Presety dla Expo",
    presetsForExpoDescription:
      "Dostarczamy presety dla Expo, abyś mógł szybko zacząć.",
    presetForReactNative: "Preset dla React Native",
    presetForReactNativeDescription:
      "Dostarczamy preset dla React Native, abyś mógł szybko zacząć.",
    presetForReactEmail: "Biblioteka React Email",
    presetForReactEmailDescription:
      "Stworzyliśmy bibliotekę i18n dla React Email.",
    readyForI18nLibraries: "Gotowe na biblioteki i18n",
    readyForI18nLibrariesDescription:
      "Wspieramy wszystkie biblioteki, w tym te najbardziej popularne (next-intl, react-i18next, react-intl, itp.).",
  },
  info: {
    title: "Zarządza każdym aspektem lokalizacji twojej aplikacji, w tym",
    smartTranslation: {
      title: "Inteligentne tłumaczenie",
      intelligentTranslation:
        "Inteligentne tłumaczenie z uwzględnieniem kontekstu",
      brandVoice: "Spójność głosu i tonu marki",
      terminology: "Zarządzanie terminologią",
      linguisticFeatures: "Zaawansowane funkcje lingwistyczne",
      realtimeUpdates: "Aktualizacje tłumaczeń w czasie rzeczywistym",
    },
    implementation: {
      title: "Implementacja",
      quickSetup: "Szybka konfiguracja i integracja",
      fileFormat: "Obsługa formatów plików",
      contentStructure: "Organizacja struktury treści",
      assetOrganization: "Organizacja zasobów",
    },
    developer: {
      title: "Doświadczenie programisty",
      cli: "Interfejs wiersza poleceń",
      cicd: "Integracja CI/CD",
      versionControl: "Kontrola wersji",
      workflow: "Przepływ pracy programisty",
      documentation: "Kompletna dokumentacja",
    },
  },
  login: {
    title: "Zaloguj się",
    github: "Zaloguj się przez GitHub",
    google: "Zaloguj się przez Google",
    footer: "Automatyzuj swoją lokalizację.",
    description:
      "Zaloguj się, aby zacząć automatyzować swoją lokalizację w kilka sekund.",
    terms: {
      text: "Logując się, zgadzasz się na nasze",
      termsOfService: "Warunki usługi",
      and: "i",
      privacyPolicy: "Politykę prywatności",
    },
  },
  userMenu: {
    account: "Ustawienia konta",
    signOut: "Wyloguj się",
    createTeam: "Utwórz zespół",
    homepage: "Strona główna",
    team: "Ustawienia zespołu",
  },
  teamSelector: {
    addProject: "Utwórz projekt",
    createProjectTitle: "Utwórz nowy projekt",
    projectNamePlaceholder: "Nazwa projektu",
    createProjectButton: "Utwórz projekt",
    pro: "Pro",
    project: "Projekt",
    teams: "Zespoły",
    createTeam: "Utwórz zespół",
    createTeamTitle: "Utwórz nowy zespół",
    teamNamePlaceholder: "Nazwa zespołu",
    createTeamButton: "Utwórz zespół",
  },
  coming_soon: {
    title: "Languine jest w fazie wczesnego dostępu",
    description: "Obecnie jesteśmy w fazie wczesnego dostępu. Skontaktuj się z",
    cta: "na X, aby uzyskać wczesny dostęp.",
  },
  account: {
    fullName: {
      title: "Pełne imię i nazwisko",
      description:
        "Twoje pełne imię i nazwisko, które będzie widoczne na platformie.",
      placeholder: "Wprowadź swoje pełne imię i nazwisko",
    },
    email: {
      title: "Adres e-mail",
      description: "Adres e-mail powiązany z twoim kontem.",
      placeholder: "Wprowadź swój adres e-mail",
    },
    apiKey: {
      title: "Klucz API",
      description:
        "Twój osobisty klucz API do dostępu do API Languine i z CLI.",
    },
    deleteAccount: {
      title: "Usuń konto",
      description:
        "Trwale usuń swoje konto i wszystkie powiązane dane. Tej akcji nie można cofnąć.",
      button: "Usuń konto",
    },
  },
  copyInstall: {
    copied: "Skopiowano do schowka",
  },
  dangerZone: {
    dialog: {
      title: "Czy jesteś absolutnie pewien?",
      description: "Tej akcji nie można cofnąć. Wpisz DELETE, aby potwierdzić.",
      placeholder: "Wpisz DELETE, aby potwierdzić",
      confirm: "Potwierdź usunięcie",
      cancel: "Anuluj",
    },
  },
  pipeline: {
    title: "Przepływ pracy (Potok CI/CD)",
    pro: "[pro]",
    description:
      "Nasz silnik tłumaczeń bezproblemowo integruje się z twoim istniejącym potokiem CI/CD, automatycznie tłumacząc twoją bazę kodu przy każdym pushu. Gdy zmiany w kodzie są pushowane, analizujemy zmodyfikowaną zawartość, utrzymujemy twoją pamięć tłumaczeń i generujemy dokładne tłumaczenia, zachowując głos i terminologię twojej marki. Tłumaczenia są następnie przesyłane jako pull requesty, umożliwiając przegląd przed włączeniem do głównej gałęzi i wdrożeniem. Ten zautomatyzowany przepływ pracy zapewnia, że twoja zlokalizowana zawartość pozostaje zsynchronizowana z rozwojem.",
  },
  settings: {
    badRequest: "Nieprawidłowe żądanie",
    badRequestDescription:
      "Organizacja musi mieć przynajmniej jednego właściciela. Przenieś własność na innego członka przed usunięciem tego właściciela.",
    permissionDenied: "Odmowa dostępu",
    permissionDeniedDescription:
      "Musisz być właścicielem zespołu, aby wykonać tę akcję",
    deleteTeamTitle: "Usuń zespół",
    deleteTeamDescription:
      "Nie można usunąć organizacji, gdy jesteś jedynym członkiem, zamiast tego usuń swoje konto",
    saved: "Ustawienia zapisane",
    savedDescription: "Twoje zmiany zostały pomyślnie zapisane",
    error: "Coś poszło nie tak",
    errorDescription: "Spróbuj ponownie lub skontaktuj się z pomocą techniczną",
    addTeam: "Utwórz zespół",
    apiKey: {
      updated: "Klucz API zaktualizowany",
      updatedDescription:
        "Twój klucz API został pomyślnie zaktualizowany i skopiowany do schowka.",
      title: "Zaktualizuj klucz API",
      description:
        "Czy na pewno chcesz wygenerować nowy klucz API? Stary klucz przestanie działać natychmiast.",
      cancel: "Anuluj",
      continue: "Kontynuuj",
    },
    tabs: {
      project: "Projekt",
      account: "Konto",
      team: "Zespół",
      provider: "Dostawca",
      billing: "Rozliczenia",
    },
    provider: {
      updateSuccess: "Ustawienia pomyślnie zaktualizowane",
      updateError: "Nie udało się zaktualizować ustawień",
      translationProvider: {
        title: "Dostawca tłumaczeń",
        description:
          "Wybierz preferowaną usługę AI do generowania tłumaczeń. Każdy dostawca oferuje różne możliwości i ceny.",
        placeholder: "Wybierz dostawcę",
      },
      languageModel: {
        title: "Model językowy",
        description:
          "Wybierz model AI, który najlepiej równoważy jakość i szybkość dla twoich potrzeb tłumaczeniowych. Bardziej zaawansowane modele mogą być wolniejsze, ale dają lepsze wyniki.",
        placeholder: "Wybierz model",
      },
      apiKey: {
        title: "Klucz API dostawcy",
        description:
          "Wprowadź swój klucz API, aby uwierzytelnić się u wybranego dostawcy. Zachowaj ten klucz w bezpiecznym miejscu - szyfrujemy go przed przechowaniem.",
        placeholder: "Wprowadź swój klucz API",
      },
    },
    addProject: "Utwórz projekt",
    project: {
      name: {
        title: "Nazwa projektu",
        description: "Nazwa twojego projektu",
        placeholder: "Wprowadź nazwę projektu",
      },
      id: {
        title: "ID projektu",
        description: "Twój unikalny identyfikator projektu",
        placeholder: "ID projektu",
      },
      delete: {
        title: "Usuń projekt",
        description: "Trwale usuń ten projekt i wszystkie jego dane",
        button: "Usuń projekt",
      },
    },
    save: "Zapisz",
    team: {
      name: {
        title: "Nazwa zespołu",
        description: "Nazwa twojego zespołu",
        placeholder: "Wprowadź nazwę zespołu",
      },
      billing: {
        title: "Plan rozliczeniowy",
        description: "Zarządzaj planem rozliczeniowym swojego zespołu",
        free: "Darmowy",
        pro: "Pro",
        unlimited: "Nieograniczony",
      },
      apiKey: {
        title: "Klucz API zespołu",
        description:
          "Klucz API dostępu zespołu, używaj go z potoku CI/CD razem z ID projektu",
        placeholder: "Klucz API zespołu",
      },
      members: {
        leaveTeamConfirm: "Opuść zespół",
        leaveTeamDescription: "Czy na pewno chcesz opuścić ten zespół?",
        cancel: "Anuluj",
        leaveTeam: "Opuść zespół",
        invite: "Zaproś członka",
        title: "Członkowie",
        pendingInvitations: "Oczekujące zaproszenia",
        removeMember: "Usuń członka",
        removeMemberConfirm: "Usuń członka",
        removeMemberDescription: "Czy na pewno chcesz usunąć tego członka?",
        filterPlaceholder: "Filtruj członków...",
        allRoles: "Wszystkie role",
        date: "Data",
        selectAll: "{count} wybranych",
        noPendingInvitations: "Brak oczekujących zaproszeń",
        noResults: "Nie znaleziono członków",
        tryDifferentSearch: "Spróbuj innego wyszukiwania",
        inviteMembers: "Zaproś członków do współpracy",
        deleteInvite: "Usuń zaproszenie",
        deleteInviteDescription: "Usuń zaproszenie",
        invitedBy: "Zaproszony przez {name}",
        deleteInviteSuccess: "Zaproszenie usunięte",
        deleteInviteError: "Nie udało się usunąć zaproszenia",
        removeMemberSuccess: "Członek usunięty",
        removeMemberError: "Nie udało się usunąć członka",
        removeMemberSuccessDescription: "Członek został pomyślnie usunięty",
        removeMemberErrorDescription: "Nie udało się usunąć członka",
        roles: {
          owner: "Właściciel",
          member: "Członek",
        },
        dateSort: {
          newest: "Najnowsze",
          oldest: "Najstarsze",
        },
      },
    },
  },
  tuning: {
    general: "Ogólne",
    translationMemory: {
      title: "Pamięć tłumaczeń",
      description:
        "Używaj pamięci tłumaczeń, aby poprawić spójność i efektywność",
    },
    qualityChecks: {
      title: "Kontrola jakości",
      description: "Włącz automatyczną kontrolę jakości tłumaczeń",
    },
    contextDetection: {
      title: "Wykrywanie kontekstu",
      description: "Automatycznie wykrywaj i zachowuj kontekst w tłumaczeniach",
    },
    styleGuide: "Przewodnik stylu",
    lengthControl: {
      title: "Kontrola długości",
      description: "Kontroluj długość przetłumaczonego tekstu",
      options: {
        flexible: "Elastyczny",
        strict: "Ścisły",
        exact: "Dokładny",
        loose: "Luźny",
      },
    },
    inclusiveLanguage: {
      title: "Język inkluzywny",
      description: "Zapewnij, że tłumaczenia używają języka inkluzywnego",
    },
    formality: {
      title: "Formalność",
      description: "Kontroluj poziom formalności tłumaczeń",
    },
    brandName: {
      title: "Nazwa marki",
      description: "Ustaw nazwę swojej marki dla spójnego użycia",
      placeholder: "Wprowadź nazwę swojej marki",
    },
    brandVoice: {
      title: "Głos marki",
      description: "Zdefiniuj głos i ton swojej marki",
      placeholder: "Opisz głos swojej marki...",
    },
    localization: "Lokalizacja",
    idioms: {
      title: "Idiomy",
      description: "Odpowiednio zarządzaj wyrażeniami idiomatycznymi",
    },
    terminology: {
      title: "Terminologia",
      description: "Zarządzaj specjalistyczną terminologią i glosariuszami",
    },
    culturalAdaptation: {
      title: "Adaptacja kulturowa",
      description: "Dostosuj zawartość do odpowiedniości kulturowej",
    },
  },
  pricing: {
    title: "Proste ceny",
    free: {
      title: "Darmowy (100 kluczy)",
      price: "Darmowy",
      keys_limit: "Do 100 kluczy",
      features: {
        unlimited_projects: "Nieograniczone projekty",
        fine_tuning: "Opcje dostosowania",
        overrides: "Nadpisania tłumaczeń",
        analytics: "Analityka",
        context_memory: "Pamięć kontekstu",
        community_support: "Wsparcie społeczności",
      },
    },
    pro: {
      title: "Pro",
      includes_free: "Wszystko z Darmowego, plus:",
      features: {
        github_action: "Integracja z GitHub Action",
        latest_features: "Wczesny dostęp do najnowszych funkcji",
        priority_support: "Priorytetowe wsparcie",
      },
    },
    cta: "Zacznij automatyzować",
  },
  createTeam: {
    teamName: "Nazwa zespołu",
    teamNamePlaceholder: "Wprowadź nazwę zespołu",
    createTeamButton: "Utwórz zespół",
    cancel: "Anuluj",
    createTeamDescription:
      "Utwórz nowy zespół, aby zarządzać swoimi projektami i współpracownikami.",
  },
  createProject: {
    createProjectTitle: "Utwórz nowy projekt",
    projectNamePlaceholder: "Wprowadź nazwę projektu",
    createProjectButton: "Utwórz projekt",
    cancel: "Anuluj",
    createProjectDescription:
      "Utwórz nowy projekt, aby zacząć automatyzować swoją lokalizację.",
  },
  invite: {
    inviteMember: "Zaproś członka",
    inviteDescription: "Zaproś członka do swojego zespołu",
    emailLabel: "E-mail",
    emailPlaceholder: "Wprowadź e-mail",
    cancel: "Anuluj",
    sendInvite: "Wyślij zaproszenie",
    success: {
      title: "Zaproszenie wysłane",
      description: "Pomyślnie wysłano zaproszenie do {email}",
    },
    error: {
      title: "Błąd",
      description: "Nie udało się wysłać zaproszenia. Spróbuj ponownie.",
    },
    validation: {
      invalidEmail: "Wprowadź poprawny adres e-mail",
    },
  },
  months: {
    "10": "Paź",
    "11": "Lis",
    "12": "Gru",
    "01": "Sty",
    "02": "Lut",
    "03": "Mar",
    "04": "Kwi",
    "05": "Maj",
    "06": "Cze",
    "07": "Lip",
    "08": "Sie",
    "09": "Wrz",
  },
  onboarding: {
    steps: {
      "1": {
        title: "Skonfiguruj Languine CLI",
        description:
          "Zainstaluj i skonfiguruj Languine CLI do zarządzania tłumaczeniami",
      },
      "2": {
        title: "Pushuj tłumaczenia",
        description: "Oczekiwanie na push tłumaczeń",
      },
      "3": {
        title: "Dokumentacja",
        description:
          "Sprawdź naszą dokumentację, aby uzyskać szczegółowe przewodniki i najlepsze praktyki",
        link: "dokumentacja",
      },
    },
    info: {
      description: "Potrzebujesz pomocy? Sprawdź naszą",
      link: "dokumentacja",
      description_2:
        "aby uzyskać szczegółowe przewodniki i najlepsze praktyki.",
    },
  },
  cli: {
    success: {
      title: "Sukces logowania Languine CLI",
      description: "Languine CLI zostało pomyślnie uwierzytelnione jako:",
      description_2: "Możesz teraz zamknąć tę kartę i wrócić do CLI",
    },
  },
  translations: {
    header: "Przetłumaczone klucze",
  },
  search: {
    placeholder: "Szukaj",
  },
  periods: {
    daily: "Dziennie",
    weekly: "Tygodniowo",
    monthly: "Miesięcznie",
    selectPeriod: "Wybierz okres",
  },
  welcome: {
    title: "Witaj w Languine",
    description: "Automatyzuj swój proces lokalizacji z Languine",
  },
  button: {
    update: "Nowe tłumaczenia",
  },
  update: {
    button: "Nowe tłumaczenia",
  },
  hero2: {
    title: "Automatyzacja lokalizacji dla Twoich aplikacji",
    description:
      "Usprawnij proces lokalizacji dzięki wspieranemu przez AI interfejsowi wiersza poleceń oraz potokowi zaprojektowanemu do automatyzacji tłumaczeń dla programistów.",
  },
  language: {
    en: "Angielski",
    es: "Hiszpański",
    fr: "Francuski",
    de: "Niemiecki",
    it: "Włoski",
    pt: "Portugalski",
    zh: "Chiński",
    ja: "Japoński",
    ko: "Koreański",
    ar: "Arabski",
    hi: "Hindi",
    tr: "Turecki",
    nl: "Holenderski",
    pl: "Polski",
    no: "Norweski",
    da: "Duński",
    sv: "Szwedzki",
    fi: "Fiński",
    vi: "Wietnamski",
  },
} as const;
